...
export default function Toolbar({ previewDevice, onDeviceChange, onExportPdf, onExportHtml, onCopy, copied, isCopying, scrollSyncEnabled, onToggleScrollSync }: ToolbarProps) {
    return (
        <div className="flex items-center justify-between px-4 sm:px-6 py-2 max-w-[1024px]">
            ...
            <div className="flex items-center gap-4">
                <motion.button
                    ...
                    className={`apple-export-btn !bg-[#00000008] dark:!bg-[#ffffff10] border-transparent text-xs !px-2 !py-1 ${scrollSyncEnabled ? 'text-[#0066cc] dark:text-[#0a84ff]' : 'text-[#86868b] dark:text-[#a1a1a6]'}`}
                    title={scrollSyncEnabled ? '关闭滚动同步' : '开启滚动同步'}
                >
                    {scrollSyncEnabled ? <Link2 size={12} /> : <Unlink2 size={12} />}
                ...
                <motion.button
                    ...
                    onClick={onExportPdf}
                    className="apple-export-btn !hidden sm:!flex !bg-[#00000008] dark:!bg-[#ffffff10] border-transparent text-xs !px-2 !py-1"
                >
                    <Download size={12} />
                    导出 PDF
                </motion.button>
                ...
                <motion.button
                    ...
                    className="apple-export-btn !hidden lg:!flex !bg-[#00000008] dark:!bg-[#ffffff10] border-transparent text-xs !px-2 !py-1"
                >
                    <Download size={12} />
                    导出 HTML
                </motion.button>
                ...
                <motion.button
                    ...
                    onClick={onCopy}
                    disabled={isCopying}
                    className={`apple-copy-btn apple-copy-btn text-xs !px-2 !py-1 ${copied ? 'apple-copy-btn-success' : isCopying ? 'opacity-80 cursor-not-allowed' : ''}`}
                >
                    {copied ? <CheckCircle2 size={12} /> : isCopying ? <Loader2 className="animate-spin" size={12} /> : <Copy size={12} />}
